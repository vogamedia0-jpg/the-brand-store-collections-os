...
import {
  ArrowDownToLine, ArrowLeft, ArrowUpRight, BarChart3,
  Check, CheckCircle2, ChevronRight, CloudUpload, Copy,
  ...
  Search, Send, Settings2, Sparkles, Sun, TrendingUp, Trash2, UploadCloud, X, Zap
} from 'lucide-react';
import {
  useHealthCheck,
  useListCollections,
...
function DashboardPage() {
  ...
  return <AdminShell><div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
    ...
    <section className="mt-5 grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] p-6"><div className="flex items-center justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">Category mix</p><h2 className="mt-2 font-display text-2xl">This collection, by edit.</h2></div><BarChart3 size={19} className="text-[hsl(var(--primary))]" /></div><div className="mt-7 space-y-4">{Object.entries(categories).map(([name, count]) => <div key={name} className="flex items-center gap-3 text-sm"><span className="w-20 text-[hsl(var(--muted-foreground))]">{categoryLabels[name] ?? name}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-[hsl(var(--muted))]"><div className="h-full rounded-full bg-[#c9a96a]" style={{ width: `${(count / Math.max(total, 1)) * 100}%` }} /></div><span className="w-6 text-right font-mono-ui text-xs">{count}</span></div>)}</div></div><div className="rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] p-6"><div className="flex items-center justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">Gender split</p><h2 className="mt-2 font-display text-2xl">Catalogue by audience</h2></div><TrendingUp size={19} className="text-[hsl(var(--primary))]" /></div><div className="mt-7 space-y-4">{[['Women', summary?.women ?? 0], ['Men', summary?.men ?? 0], ['Unsorted', summary?.unknown ?? 0]].map(([label, count]) => <div key={String(label)} className="flex items-center gap-3 text-sm"><span className="w-20 text-[hsl(var(--muted-foreground))]">{label}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-[hsl(var(--muted))]"><div className="h-full rounded-full bg-[hsl(var(--primary))]" style={{ width: `${(Number(count) / Math.max(total, 1)) * 100}%` }} /></div><span className="w-6 text-right font-mono-ui text-xs">{count}</span></div>)}</div></div></section>
  </div></AdminShell>;
}
  ...
  return <div className="min-h-[100dvh] bg-[#f8f5ed] text-[#0b1f44]">
    <header className="border-b border-[#d8c8ad] bg-[#f8f5ed]">
      <div className="mx-auto flex h-[82px] max-w-[1320px] items-center justify-between px-5 sm:px-8">
        <Link href="/catalogue" aria-label="The Brand Store home"><AppLogo /></Link>
      ...
      <section className="mx-auto max-w-[1320px] px-5 pb-10 pt-6 sm:px-8 sm:pb-14 sm:pt-10">
        <div className="relative min-h-[430px] overflow-hidden bg-[#0b1f44] sm:min-h-[560px]">
          ...
          <div className="relative flex min-h-[430px] max-w-[560px] flex-col justify-end px-7 py-9 text-[#f8f5ed] sm:min-h-[560px] sm:px-14 sm:py-14">
            <img src={logoDark} alt="The Brand Store" className="mb-8 h-auto w-48 object-contain object-left sm:w-56" />
            <h1 className="max-w-[440px] font-display text-5xl leading-[.98] tracking-[-.035em] sm:text-7xl">Luxury lives here.</h1>
            <p className="mt-5 max-w-[380px] text-sm leading-6 text-[#f8f5ed]/80">The Brand Store catalogue.</p>
...
function AnalyticsPage() {
  ...
  return <AdminShell><div className="mx-auto max-w-[1120px] px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
    <PageIntro eyebrow="Analytics" title="Catalogue analytics" description="Review catalogue volume, review status and publishing progress." />
