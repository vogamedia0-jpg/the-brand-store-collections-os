...
const logo = 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/TB-GOLD-NB-z3H8Er1wE5LuyiaxEkDkhHHI4gaYCF.png';
  const lightLogo = 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/TB-LOGO%20LIGHT-NB-d0YFlcmN9D1ujBxqwR2Htzfc8u1sJG.png';
...
function AppLogo({ dark = false }: { dark?: boolean }) {
  return <img src={dark ? lightLogo : logo} alt="The Brand Store — Luxury lives here" className="h-10 w-auto object-contain" />;
}
...
function AdminLogin() {
  ...
  return <div className="flex min-h-[100dvh] items-center justify-center bg-[hsl(var(--sidebar))] px-5 py-10 text-[hsl(var(--sidebar-foreground))] noise">
    <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/[.06] p-7 shadow-2xl sm:p-10">
      ...
      <p className="mt-3 text-sm leading-6 text-white/60">Sign in to manage The Brand Store collection.</p>
  ...
  return <AuthenticatedAdmin><div className="min-h-[100dvh] bg-[hsl(var(--background))] noise">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[272px] -translate-x-full flex-col bg-[hsl(var(--sidebar))] px-5 py-6 text-[hsl(var(--sidebar-foreground))] transition-transform duration-300 lg:translate-x-0 ${open ? 'translate-x-0' : ''}`}>
      ...
      <div className="mb-8 rounded-xl border border-white/10 bg-white/[.05] p-3">
        <p className="font-mono-ui text-[9px] uppercase tracking-[.18em] text-[#C9A96A]">Active collection</p>
...
function ProductCard({ product, onDelete }: { product: Product; onDelete?: (id: string) => void }) {
  return <article className="group overflow-hidden rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))]"><Link href={`/catalogue/product/${product.id}`} className="relative block aspect-[.85] overflow-hidden bg-[#ded2c5]"><img src={imageFor(product)} alt={`${product.brand || 'The Brand Store'} ${categoryLabels[product.category]}`} className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" /><div className="absolute left-3 top-3"><Badge tone={product.isPublished ? 'success' : 'neutral'}>{product.isPublished ? 'Live' : 'Draft'}</Badge></div><button type="button" onClick={(event) => { event.preventDefault(); onDelete?.(product.id); }} aria-label={`Delete product ${product.id}`} className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/20 text-white opacity-0 transition group-hover:opacity-100" data-testid={`button-delete-product-${product.id}`}><Trash2 size={14} /></button></Link><div className="p-4"><div className="flex items-center justify-between gap-2"><div><p className="font-mono-ui text-[9px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">{genderLabels[product.gender]} / {categoryLabels[product.category]}</p><h3 className="mt-1 font-display text-xl">{product.brand || 'House edit'}</h3></div><button type="button" aria-label={`Edit product ${product.id}`} className="rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid={`button-edit-product-${product.id}`}><MoreHorizontal size={16} /></button></div></div></article>;
}
    ...
    <div className="mx-auto max-w-[1120px] px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
      ...
      <section className="mb-5 overflow-hidden rounded-2xl bg-[hsl(var(--sidebar))] text-white">
        <div className="grid md:grid-cols-[1fr_.8fr]">
          ...
          <div className="relative min-h-[260px]"><img src={boardImage} alt="The Brand Store brand materials" className="absolute inset-0 h-full w-full object-cover opacity-65" /><div className="absolute inset-0 bg-gradient-to-r from-[hsl(var(--sidebar))] via-transparent to-transparent" /></div>
        </div>
      </section>
...
function SettingsPage() {
  ...
  const [form, setForm] = useState({ whatsappNumber: data?.whatsappNumber || import.meta.env.VITE_LUXE_HORIZON_WHATSAPP || '', businessName: data?.businessName || 'The Brand Store', tagline: data?.tagline || 'Luxury lives here.' });
